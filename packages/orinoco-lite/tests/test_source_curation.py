...
from orinoco_lite.source_adapters import curation as HOST
...
def write_workspace(root: Path, records: str = TEST_RECORD_ROOT) -> None:
    (root / "orinoco.yaml").write_text(
        f"contract_version: 2\npaths:\n  records: {records}\n"
    )
    (root / "site-specific").mkdir(exist_ok=True)
    (root / "site-specific/site.yaml").write_text(
        "version: 1\nidentity:\n  title: Curation fixture\n"
        "  description: Curation tests.\n"
...
""",
            ...
            (metadata_root / "site-specific").mkdir()
            (metadata_root / "site-specific/site.yaml").write_text(
                "version: 1\nidentity:\n"
                "  title: Candidate fixture\n"
                "  description: Candidate path fixture.\n"
                "  base_url: https://example.invalid/fixture/\n",
                encoding="utf-8",
            )
...
class ProposalValidationBoundaryTests(unittest.TestCase):
    def test_stage_success_requires_structural_and_joined_validation(self) -> None:
        ...
        resources_root = Path("/tmp/package-resources")
        workspace = object()
        calls: list[str] = []
        ...
        with (
            ...
            mock.patch.object(HOST, "load_lock"),
            mock.patch.object(
                HOST, "resolve_resources",
                return_value=mock.Mock(root=Path("/tmp/package-resources")),
            ),
            mock.patch.object(
                HOST,
                "validate_semantics",
            ...
            changed = HOST.stage_validated_plan(root, value)

        self.assertEqual(("changed.yaml",), changed)
        self.assertEqual(["stage", "load", "structural", "joined"], calls)
        semantic.assert_called_once_with(workspace, resources_root)
...
class ProposalCommitTests(unittest.TestCase):
    ...
    def test_custom_roots_survive_proposal_review_and_finalization(self) -> None:
        records = "site-specific/custom/records"
        annotations = "site-specific/custom/overlays/annotations"
        initial = plan()
        assertion = {
            "predicate": "dcterms:title",
            "schema_type": "dlthings:AttributeSpecification",
            "value": "Reviewed new title",
        }
        accepted = replace(
            initial.candidates[0],
            proposed_record={**dict(initial.candidates[0].proposed_record), "attributes": [assertion]},
            proposed_companion=annotation_companion(
                initial.candidates[0].pid,
                [{
                    "path": "/attributes",
                    "assertion_sha256": assertion_sha256(assertion),
                    "pav:importedBy": initial.adapter_agent_pid,
                    "pav:importedFrom": "https://example.test/source/first",
                }],
            ),
            record_root=records, annotation_root=annotations,
        )
        deferred = replace(
            initial.candidates[1], record_root=records, annotation_root=annotations
        )
        with tempfile.TemporaryDirectory() as name:
            root = Path(name)
            self.git(root, "init", "-b", "main")
            self.git(root, "config", "user.name", "Fixture")
            self.git(root, "config", "user.email", "fixture@example.test")
            write_workspace(root, records)
            for change in accepted.file_changes():
                if change.baseline is not None:
                    path = root / change.path
                    path.parent.mkdir(parents=True, exist_ok=True)
                    path.write_bytes(change.baseline)
            self.git(root, "add", "-A")
            self.git(root, "commit", "-m", "test: create custom metadata base")
            base = self.git(root, "rev-parse", "HEAD")
            value = replace(initial, metadata_base=base, candidates=(accepted, deferred))
            changed = HOST.stage_plan(root, value)
            self.assertIn(accepted.companion_repository_path, changed)
            self.assertFalse((root / TEST_RECORD_ROOT).exists())
            self.git(root, "add", "-A")
            self.git(root, "commit", "-m", "test: capture custom metadata proposal")
            proposal = self.git(root, "rev-parse", "HEAD")
            HOST.verify_proposal_commit(root, value, proposal)
            accepted_path = root / accepted.record_repository_path
            accepted_path.write_text(
                accepted_path.read_text() + "description: Human correction\n"
            )
            self.git(root, "add", "-A")
            self.git(root, "commit", "-m", "test: retain a human correction")
            head = self.git(root, "rev-parse", "HEAD")
            self.assertEqual(
                1, HOST.validate_review_history(root, proposal, head, base_root=root)
            )
            payload = submission_payload(value)
            payload.update(proposal_sha=proposal, head_sha=head)
            submission = HOST.parse_submission_comment(comment(payload))
            with mock.patch.object(HOST, "_validate_joined_workspace"):
                result = HOST.finalize_review(
                    root, dry_run=False, plan=value, submission=submission,
                    repository="con/example", pull_request=17,
                    proposal_sha=proposal, head_sha=head,
                    reviewer="https://github.com/reviewer",
                    reviewed_at="2026-08-24T12:00:00Z",
                    review_url="https://github.com/con/example/pull/17#issuecomment-123",
                    review_ref="github-comment:123",
                    base_cache=HOST.DecisionCache.empty(value.adapter),
                )
            self.assertTrue(result["metadata_changed"])
            self.assertIn("Human correction", accepted_path.read_text())
            self.assertTrue((root / accepted.companion_repository_path).is_file())
            self.assertFalse((root / deferred.record_repository_path).exists())
            self.assertTrue((root / HOST.DECISION_CACHES[value.adapter]).is_file())

    def test_declared_roots_reject_an_outside_plan_before_any_write(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            root = Path(name)
            self.git(root, "init", "-b", "main")
            self.git(root, "config", "user.name", "Fixture")
            self.git(root, "config", "user.email", "fixture@example.test")
            write_workspace(root, "site-specific/custom/records")
            self.git(root, "add", "-A")
            self.git(root, "commit", "-m", "test: configure custom roots")
            value = replace(plan(), metadata_base=self.git(root, "rev-parse", "HEAD"))
            with self.assertRaisesRegex(HOST.CurationHostError, "configured metadata roots"):
                HOST.stage_plan(root, value)
