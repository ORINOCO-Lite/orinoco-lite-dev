...
WORKFLOW = ROOT / ".github/workflows/orinoco-curation-review.yml"
...
class CurationReviewWorkflowTests(unittest.TestCase):
    ...
    def test_triggers_and_permissions_are_narrow(self) -> None:
        self.assertEqual(
            {"workflow_call"}, set(self.contract["on"])
        )
        inputs = self.contract["on"]["workflow_call"]["inputs"]
        self.assertNotIn("adapter_agent_pid", inputs)
        self.assertEqual("false", inputs["acknowledge_public_review_data"]["required"])
    ...
    def test_proposal_derives_one_reviewed_provenance_identity_from_policy(
        ...
        self.assertIn("orinoco_lite.source_adapters.review", resolution["run"])
    ...
    def test_token_created_writes_dispatch_full_validation(
        self,
    ) -> None:
        ...
        reviewed_step = self.submit_steps[
            "Dispatch full validation for the exact reviewed ref"
        ]
    ...
    def test_proposal_is_one_inline_datalad_commit_with_both_metadata_outputs(
        ...
        self.assertIn('-o "$RECORDS_ROOT"', step)
        self.assertIn('-o "$ANNOTATIONS_ROOT"', step)
        ...
        self.assertIn("--no-renames", step)
        self.assertIn('{"A", "M", "D"}', step)
        self.assertIn("load_workspace()", step)
        self.assertIn('workspace.paths["records"]', step)
        self.assertIn("annotation_root(workspace)", step)
    ...
    def test_submission_uses_trusted_code_and_exact_authenticated_context(self) -> None:
        ...
        self.assertIn("-m orinoco_lite.source_adapters.curation", parse)
    ...
    def test_finalization_uses_the_installed_package_from_trusted_code(self) -> None:
        for name in (
            ...
            self.assertIn("-m orinoco_lite.source_adapters.curation", execution)
            self.assertIn("--trusted-root", execution)

    def test_source_checkout_is_bound_to_trusted_policy_and_exact_submission(self) -> None:
        proposal = self.propose_steps["Check out the selected dump source as data"]
        final = self.submit_steps["Check out the exact submitted dump source as data"]
        coordinates = self.submit_steps["Inspect the submitted proposal coordinates"]["run"]
        self.assertEqual(
            "${{ steps.source_policy.outputs.repository }}",
            proposal["with"]["repository"],
        )
        self.assertEqual(
            "${{ steps.proposal.outputs.source_repository }}",
            final["with"]["repository"],
        )
        self.assertEqual(
            "${{ steps.proposal.outputs.source_revision }}", final["with"]["ref"]
        )
        self.assertIn("resolve_source_repository", coordinates)
        self.assertIn('Path("trusted/site-specific/sources")', coordinates)
        self.assertIn(
            'coordinate.get("repository") != f"https://github.com/{source_repository}"',
            coordinates,
        )
        self.assertIn('r"[0-9a-f]{40}", revision', coordinates)

    def test_final_path_guard_accepts_configured_roots_and_rejects_other_files(self) -> None:
        step = self.submit_steps["Validate the complete joined graph"]
        self.assertEqual(
            "${{ steps.base.outputs.root }}", step["env"]["CURATION_BASE_ROOT"]
        )
        script = step["run"].split("python -c '\n", 1)[1].rsplit("\n'", 1)[0]
        cache = "site-specific/curation-records/zotero.yaml"
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "site-specific").mkdir()
            ...
            for parent in ("site-specific/metadata", "site-specific/custom"):
                with self.subTest(parent=parent):
                    (root / "orinoco.yaml").write_text(
                        f"contract_version: 2\npaths:\n  records: {parent}/records\n"
                    )
                    paths = (
                        f"{cache}\n{parent}/records/record.yaml\n"
                        f"{parent}/overlays/annotations/record.yaml\n"
                    )
                    environment = {
                        **os.environ,
                        "CURATION_BASE_ROOT": str(root),
                        "CACHE_PATH": cache,
                    }
                    accepted = subprocess.run(
                        [sys.executable, "-c", script], input=paths,
                        text=True, capture_output=True, env=environment,
                    )
                    self.assertEqual(0, accepted.returncode, accepted.stderr)
                    rejected = subprocess.run(
                        [sys.executable, "-c", script], input=paths + "README.md\n",
                        text=True, capture_output=True, env=environment,
                    )
                    self.assertNotEqual(0, rejected.returncode)
                    self.assertIn("unexpected path", rejected.stderr)
    ...
    def test_metadata_finalization_is_datalad_recorded_but_cache_only_is_not(
        ...
        self.assertIn('-o "$RECORDS_ROOT"', metadata)
        self.assertIn('-o "$ANNOTATIONS_ROOT"', metadata)
