...
def resolve_source_repository(source_id: str, path: Path) -> str:
    """Resolve the reviewed GitHub repository from the adapter's source policy."""
    ...
    for source in load_config(path):
        ...
        if not isinstance(repository, str) or re.fullmatch(
            ...
            raise MetadataReviewError(
                f"Source {source_id} repository must identify OWNER/REPOSITORY"
            )
        return repository
    raise MetadataReviewError(f"Unknown metadata source {source_id!r}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config = args.config or root / "site-specific/sources"
    build = root / "build/metadata-review"
    if args.mode in {"resolve-provenance-identity", "resolve-source-repository"}:
        if args.adapter is None:
            parser.error(f"{args.mode} requires --adapter")
        if args.only or args.source_input:
            parser.error(
                f"{args.mode} does not accept --only or --source-input"
            )
        try:
            resolve = (
                resolve_provenance_identity
                if args.mode == "resolve-provenance-identity"
                else resolve_source_repository
            )
            print(resolve(args.adapter, config))
        except MetadataReviewError as error:
            parser.exit(1, f"metadata-review: {error}\n")
        return 0
    if args.adapter is not None:
        parser.error("--adapter is only valid for a source-policy lookup")
