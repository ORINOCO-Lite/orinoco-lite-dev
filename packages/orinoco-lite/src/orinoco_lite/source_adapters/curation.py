...
def _relative_record_path(value: object) -> str:
    ...
    if (
        ...
        raise CurationHostError("Record path must be a normalized relative YAML path")
    return rendered
...
def _metadata_roots(root: Path) -> tuple[PurePosixPath, PurePosixPath]:
    workspace = load_workspace(root)
    return (
        PurePosixPath(workspace.paths["records"]),
        PurePosixPath(annotation_root(workspace).relative_to(workspace.root).as_posix()),
    )


def _validate_plan_roots(
    root: Path, plan: CandidatePlan
) -> tuple[PurePosixPath, PurePosixPath]:
    roots = _metadata_roots(root)
    for candidate in plan.candidates:
        if (
            PurePosixPath(candidate.record_root),
            PurePosixPath(candidate.annotation_root),
        ) != roots:
            raise CurationHostError("Candidate paths differ from configured metadata roots")
    return roots
...
def validate_review_history(
    root: Path, proposal_sha: str, head_sha: str, *, base_root: Path
) -> int:
    ...
    roots = _metadata_roots(base_root)
    history = str(
        _run_git(
            root,
            "rev-list",
    ...
    for entry in history:
        ...
        for parent in parents:
            for status, rendered in _diff_entries(root, parent, commit):
                ...
                if not any(allowed in path.parents for allowed in roots):
                    raise CurationHostError(
                        f"Review history changes code or workflow-owned state: {path}"
                    )
                _safe_metadata_path(rendered)
    return len(history)
...
def _safe_destination(
    root: Path, relative: str, roots: tuple[PurePosixPath, PurePosixPath]
) -> Path:
    path = PurePosixPath(_safe_metadata_path(relative))
    if not any(allowed in path.parents for allowed in roots):
        raise CurationHostError(f"Metadata output is outside configured roots: {relative}")
...
def stage_plan(root: Path, plan: CandidatePlan) -> tuple[str, ...]:
    ...
    roots = _validate_plan_roots(root, plan)
    changes = plan.file_changes()
    if not changes:
        return ()
    destinations = {
        change.path: _safe_destination(root, change.path, roots) for change in changes
    }
...
def stage_validated_plan(
    ...
    _validate_joined_workspace(root)
    return changed


def _validate_joined_workspace(root: Path) -> None:
    """Run shared structural and joined semantic validation without projection."""

    workspace = load_workspace(root)
    validate_workspace(workspace)
    resources = resolve_resources(workspace, load_lock(root / "orinoco.lock"))
    validate_semantics(workspace, resources.root)
...
def _load_provider(trusted_root: Path, adapter: str) -> ModuleType:
    ...
    from orinoco_lite.source_adapters.review import load_config, safe_repo_path

    configured = load_config(trusted_root / "site-specific/sources")
    source = next((item for item in configured if item["id"] == adapter), None)
    if source is None:
        raise CurationHostError(f"Trusted adapter configuration is missing: {adapter}")
    implementation = source.get("candidate_provider")
    if not isinstance(implementation, str) or not implementation:
        raise CurationHostError(f"Trusted candidate provider is missing: {adapter}")
    if implementation.startswith("orinoco_lite.source_adapters."):
        return importlib.import_module(implementation)
    path = safe_repo_path(trusted_root, implementation, label="candidate provider")
...
def _schema(root: Path) -> SchemaView:
    workspace = load_workspace(root)
    resources = resolve_resources(workspace, load_lock(root / "orinoco.lock"))
    schema_path = resources.root.joinpath(*SCHEMA_PATH.parts)
...
def _apply_finalization(
    ...
    *,
    ...
    _validate_plan_roots(review_root, plan)
    ...
    _validate_joined_workspace(review_root)
    counts = {
...
def _parser() -> argparse.ArgumentParser:
    ...
    history.add_argument("--base-root", type=Path, required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "stage-proposal":
            changed = stage_validated_plan(args.root, plan)
            _write_json(
                {
                    "adapter": plan.adapter,
                    "candidate_count": len(plan.candidates),
