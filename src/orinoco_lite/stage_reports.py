...
def _copy_artifact(source: Path, destination: Path, *, allow_failed: bool = False) -> dict:
    ...
    if source.name == "intermediate.rdf" and (source.parent / "conversion.json").is_file():
        if read_json(source.parent / "conversion.json").get("rdf_format") == "trig":
...
def write_report(report_dir: Path, *, stage: str, left: Path, right: Path,
    ...
    for index, finding in enumerate(findings, start=1):
        rows.append({**finding, "id": f"{stage}:{index}", "stage": stage,
                     "first_boundary": finding.get("first_boundary", stage),
