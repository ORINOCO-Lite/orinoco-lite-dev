...
def test_cli_bundle_and_serve_respect_root(tmp_path, monkeypatch, capsys):
    ...
    write_report(tmp_path / 'sourcedata/reports/downloaded-vs-yaml-jsonl', stage='storage', left=tmp_path / 'data.json',
                 right=tmp_path / 'data.json', comparator='records-v1', findings=[])
    assert cli.main(['--root', str(tmp_path), 'dev', 'review', 'bundle', 'downloaded-vs-yaml-jsonl',
                     '--title', 'CLI portability']) == 0
    assert ReviewModel(tmp_path / 'sourcedata/bundle').title == 'CLI portability'
    seen = []
