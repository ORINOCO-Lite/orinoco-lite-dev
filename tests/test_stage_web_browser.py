...
def test_draft_preview_export_changed_conflict_and_absent_workflows(tmp_path, browser_node, capsys):
    ...
    assert.match(await page.locator('#stage-flow button').filter({hasText:'RDF'}).innerText(),/incomplete/);
...
""", conflictId=conflicting["id"])
    ...
    change_path = tmp_path / "exported-changes.json"
    write_json(change_path, changes)
    assert stage_review.execute(Namespace(review_command="apply", changes=change_path,
                                         decisions=decision_path, write=True)) == 0
