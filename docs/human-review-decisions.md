...
| HR-233 | Treat the downstream **Propose via GitHub** click as the final authorization for one SHACL proposal, with its repository and selected records already visible. Do not add a second confirmation in the popup. Open a new draft without curator-attributed explanatory text; have trusted workflow automation post the concise waiting status as `github-actions[bot]`. Preserve unchanged RDF multivalue ordering during YAML materialization so a focused edit stays focused. | M6-D010 | John Lee | 2026-08-31 |
