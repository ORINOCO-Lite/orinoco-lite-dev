...
3. Generate fresh downstreams from the candidate template, copy in the retained site data and extensions, and test them locally.
...
Add one Pixi task that applies a local engine build, a local template checkout, or both to a temporary copy of a selected downstream.
...
For this rebuild, `leej3/orinoco-lite-demo` supplies the current site data and source-adapter extensions to retain.
Validate the joined graph, route inventory, static build, and edit and review flows before replacing the old generated tree.
Use the same generated structure for the organization reference downstream through its normal review path.
...
The downstream update may replace the generated tree wholesale; retain each downstream's current site data and extensions rather than the old framework layout.
...
2. a full local run exercises retained site data, source adapters, validation, build, and edit and review flows;
...
4. `leej3/orinoco-lite-demo` and the organization reference downstream are generated afresh from the template, build and deploy with their current site data and source-adapter extensions, and the organization update remains human-gated;
