...
| M5-D014 | SHACL Vue proposal editing is a distinct human-edit profile, not an input to the hosted decision-review bundle or finalizer. The normal version 2 editor bundle is exposed unchanged to a thin Orinoco wrapper. An explicit authenticated proposal writes one temporary bundle-only handoff commit; trusted default-branch Python replaces only that exact commit with the equivalent attributed canonical metadata commit. | The shared App adds contents write, confined to same-repository human proposal branches at exact coordinates. The final branch retains no bundle, every earlier proposal/review commit survives, and SHACL Vue gains no GitHub, adapter, disposition, provenance, or cache semantics. A public host may retain unreachable handoff objects, so only public-approved data may use this path. |
