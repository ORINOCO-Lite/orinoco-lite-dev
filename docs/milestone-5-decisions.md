...
## Compatibility decision under review

| ID | Evidence and preferred direction | Gate |
| --- | --- | --- |
| M5-Q001 | Pinned `update_data_property()` preserves an existing topical scalar and represents a differing machine value as a qualified `AttributeSpecification` or `Statement`. A scalar-only companion cannot carry both values. The preferred resolution is to store that upstream qualified assertion as canonical record content and split only its PAV into the existing companion. | Review the exact stored shapes, first-run metadata effect, projection effect, and parity matrix in the normative specification before changing the join or either adapter. Until then, affected candidate generation fails closed. |

## Refined inherited defaults

M5-D001 through M5-D011 refine M4-I003, M4-I008, M4-I014, and M4-I015 and implement HR-201, HR-207, HR-208, and HR-209 for source-adapter operation.
M5-Q001 is not yet an accepted implementation decision; it records the compatibility evidence and the user's preferred direction without treating that preference as approval of the detailed storage contract.
