...
| Nine broken HTML-link references and 790 unresolved graph paths, shared by both | **Inherited upstream behavior; later, prioritizing exposed links.** Examples: ORINOCO's UUID links to named tools and graph links to filtered-out people/publications. Graph records and page selectors have different scopes. Matching upstream proves inheritance, not correctness; never suppress all broken links. |
...
Use `--site-specific /path/to/inputs` for prepared site inputs instead of snapshot conversion.
