...
| `dump-research-info` source correction | ready pull request [`con/dump-research-info#26`](https://github.com/con/dump-research-info/pull/26), base `397b56080d14aeb94592f308b2f0bf325a298c13`, head `0477de925168d6c19496ab9b8489205026eca696`, source tree `69451a599ca11d6a490ab3841d53496b42095495` | Removed 70 presentation-routing `about` values, retained qualified homepages, and added the CON homepage as a qualified `foaf:homepage`. `uv run --group test pytest` passed 26 tests, mypy passed, and pull-request CI passed on Linux, macOS, and Windows. A 79-candidate plan now completes; joined validation next stops independently at dangling `creator` target `rrid:SCR_002630`, so no proposal was published. |
...
Exit remains pending on review and merge of the separate corpus normalization, an authenticated human review/finalization and same-source rerun, one explicit SHACL handoff, merge of the upstream routing correction, and a reviewed resolution plus proposal for the independent `dump-research-info` identifier-creator blocker.
