...
| M4-I014 | Generate projection output during validation and build, keep it ignored, and review source metadata rather than committed derivatives. | Reintroduce committed projection only if it establishes a review property that source diffs, deterministic generation, and build verification cannot provide. |
